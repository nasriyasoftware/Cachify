...
export const records = {
    ...
    sortBy: {
        oldest: <T extends CacheRecord>(maps: Map<string, Map<string, T>>) => {
            const arr = records.toArray(maps);
                ...
                for (const [_, scopeMap] of records) {
                    for (const [_, record] of scopeMap) {
                        ...
                        if (diff > policy.maxIdleTime) {
                            eventsManager.emit.evict(record as any, { reason: 'idle' });
                        }
                    }
                }
            }
            ...
            const eviction = Object.seal({
                ...
                async next() {
                    return new Promise<void>(async (resolve, reject) => {
                        if (!this.hasNext()) { return resolve() }
                        const item = this.items.shift()!;

                        let timeout: NodeJS.Timeout | null = null;
                        const RemoveHandler = async (event: RemoveEvent<CacheRecord>) => {
                            if (event.item.scope === item.scope && event.item.key === item.key) {
                                evicted++;
                                (eventsManager as any).removeHandler('remove', RemoveHandler);
                        ...
                        (eventsManager as any).on('remove', RemoveHandler);

                        // Emit the eviction
                        (eventsManager as any).emit.evict(item, { reason: mode });

                        // Failsafe timeout
                        timeout = setTimeout(() => {
                            const error = new Error(`Failed to evict record: ${item.flavor}:${item.scope}:${item.key}`);
                            (eventsManager as any).removeHandler('remove', RemoveHandler);
                            reject(error);
                        }, 5000);
                    })
...
export const estimateValueSize = (value: unknown, seen = new WeakSet()): number => {
    ...
    if (typeof value === 'object') {
        ...
        if (Array.isArray(value)) {
            for (const item of value) {
                bytes += estimateValueSize(item, seen);
            }
        } else if (value instanceof Map) {
            for (const [k, v] of value.entries()) {
                bytes += estimateValueSize(k, seen);
                bytes += estimateValueSize(v, seen);
            }
            ...
            for (const [key, val] of Object.entries(value)) {
                bytes += key.length * 2;
                bytes += estimateValueSize(val, seen);
            }
        }
